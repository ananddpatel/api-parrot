...
let Parrot = function(app, options = {}) {
  ...
  app.use(serveStatic(path.join(__dirname, 'parrot-ui')));
...
function parrotRoutesSetup(app, cage) {
  ...
  app.post('/parrot/toggle', toggleRoutes(cage));
}
...
let setReqResInCage = function(cage) {
  return function(req, res) {
    try {
      cage.set(req.body.route, req.body.squawk, req.body.options);
      res.send(cage.data);
    } catch (error) {
...
/**
 * Togles disabled and enabled routes in Cage to bypass or send mock reponse
 * @param {Cage} cage
 */
let toggleRoutes = function(cage) {
  return function(req, res) {
    try {
